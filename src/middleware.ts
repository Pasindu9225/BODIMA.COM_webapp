...
export const runtime = "nodejs"; // Add this to the top!
...
export async function middleware(req: NextRequest) {
  ...
  const { role, status } = token as any; // Cast token to get role/status
...
export const config = {
  matcher: ["/login", "/register/:path*", "/provider/:path*", "/admin/:path*"],
};
